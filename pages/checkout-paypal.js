...
export default function OrderCheckout() {
  ...
  useEffect(() => {
    if (
      coupon != null &&
      coupon.discount_type == "POR CIENTO" &&
      cart != null
    ) {
      setPrice(
        parseFloat(
      ...
      coupon != null &&
      coupon.discount_type == "FIJO" &&
      cart != null
    ) {
      setPrice(
        parseFloat(
          cart.cost - coupon.discount + (cart.cost - coupon.discount) * 0.07
        ).toFixed(2)
      );
    } else if (cart != null) {
  ...
  return (
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        <h1 className={styles.header_title}>Make Payment</h1>
            ...
            <div className={styles.paypal_div}>
              ...
              <PayPalButton
                ...
                onSuccess={(details, data) => {
                  ...
                  axios
                    .post(
                      domain + `store/checkout-paypal/${cart.token}/`,
                      body,
                      config
                    )
                    .then((res) => {
